...
class CoverTab(BaseTab):
    ...
    def _input_api_key(self):
        api_key = simpledialog.askstring("AI API Key", "Masukkan AI_API_KEY (Gemini API Key):", parent=self, show="*")
        if api_key is None:
            return
    ...
    def _open_modul_input_popup(self):
        ...
        ttk.Label(wrapper, text="Pilih file modul (hanya PDF atau DOC)").pack(anchor="w", pady=(0, 8))

        path_row = ttk.Frame(wrapper)
        path_row.pack(fill="x")
        path_row.columnconfigure(0, weight=1)

        ttk.Entry(path_row, textvariable=selected_path, state="readonly").grid(row=0, column=0, sticky="ew")
        ...
        ttk.Button(path_row, text="...", width=3, command=browse_file).grid(row=0, column=1, padx=(6, 0))
